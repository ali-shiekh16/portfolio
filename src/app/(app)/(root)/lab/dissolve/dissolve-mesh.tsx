import { simplexNoise3d } from "@/lib/tsl-utils/webgpu/noise/simplex-noise-3d";
...
export class DissolveMesh extends Mesh<BufferGeometry, NodeMaterial> {
  ...
  constructor(
    ...
    const mappedProgress = this.uniforms.progress
      .remap(0, 1, -1, 1)
    ...
    const particlesMesh = new InstancedMesh(
      new PlaneGeometry(),
      particlesMaterial,
      count
    );
    ...
    particlesMaterial.positionNode = Fn(() => {
      ...
      newVelocity.addAssign(
        vec3(xwave1.add(xwave2), 1, 0).mul(
          deltaTime.mul(this.uniforms.particles.speed)
        )
      );
      ...
      const distanceDecay = basePosition
        .distance(position)
        .remapClamp(0, 1, decayDistance, 1);
      newLife.assign(
        life.add(deltaTime.mul(decayFrequency).mul(distanceDecay))
      );
