...
import BaseExperience from "@/app/(app)/(docs)/components/webgl/base-experience";
import { DissolveMesh } from "./dissolve-mesh";
...
class Dissolve extends BaseExperience {
  ...
  initTweakPane() {
    ...
    // @ts-ignore
    this.progressButton = this.tweakPane.addButton({
    ...
    if (
      !matchMedia("(any-hover: hover), (hover: hover) and (pointer: fine)")
        .matches
    ) {
    ...
    // @ts-ignore
    this.tweakPane.addBinding(
      this.mesh.uniforms.particles.decayFrequency,
      "value",
      {
        min: 0,
        max: 2,
