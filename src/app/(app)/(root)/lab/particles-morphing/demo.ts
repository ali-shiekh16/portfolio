...
const models = ["/models/face2.glb", "/models/suzanne.glb"];
...
class Demo {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    Promise.all(models.map((url) => gltfLoader.loadAsync(url))).then(
      (gltfs) => {
        const getMesh = (scene: Object3D, index: number) => {
          if (index === 0) {
            return scene.children[1].children[0].children[0]
              .children[0] as Mesh;
          }
        ...
        const maxCount = Math.max(
          ...gltfs.map(
            (gltf, i) =>
              getMesh(gltf.scene, i).geometry.attributes.position.count
          )
        );
        ...
        gltfs.forEach((gltf, i) => {
          ...
          for (let i = 0; i < maxCount; i++) {
            ...
            if (i < originalPositions.length / 3) {
              ...
              const randomIndex =
                Math.floor(
                  mesh.geometry.attributes.position.count * Math.random()
                ) * 3;
              prepositions[i] = [
  ...
  morph() {
    if (this.mesh) {
      this.mesh.setActiveIndex(
        this.mesh.uniforms.activeIndex.value === 0 ? 1 : 0
      );
    }
  }
