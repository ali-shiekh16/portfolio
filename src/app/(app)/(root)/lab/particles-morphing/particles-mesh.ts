...
import { curlNoise4d } from "@/lib/tsl-utils/webgpu/noise/curl-noise-4d";
import { Pointer } from "@/lib/tsl-utils/webgpu/pointer";
import { rotate3dY } from "@/lib/tsl-utils/webgpu/nodes/rotate-3d-y";
  ...
  constructor(
    ...
    this.buffers.basePositions = storage(
      new StorageInstancedBufferAttribute(
        new Float32Array(positions.flat() as unknown as ArrayBufferLike, 3),
        3
      ),
      "vec3",
    ...
    material.scaleNode = float(
      hash(instanceIndex).mul(0.5).add(0.5).mul(0.1)
    ...
    material.colorNode = Fn(() => {
      ...
      const color = mix(
        mainColor,
        this.buffers.colors!.element(0),
        velocity.length().mul(30).pow(2).clamp(0, 1)
      );
    ...
    this.updateCompute = Fn(() => {
      ...
      const basePosition = buffer
        .element(
          select(
            this.uniforms.activeIndex.equal(0),
            instanceIndex,
            instanceIndex.add(this.amount)
          )
        )
        .toVar();
        ...
        basePosition.assign(
          basePosition
            .mul(
              rotate3dY(
                // @ts-ignore
                float(-0.35).add(float(0.7).mul(this.uniforms.activeIndex))
              )
            )
            .add(
              vec3(
                float(-offset).add(
                  float(offset * 2).mul(this.uniforms.activeIndex)
                ),
                0,
                0
              )
      ...
      const wiggle = curlNoise4d(
        //  @ts-ignore
        vec4(position.mul(rotate3dY(time)), time.mul(this.uniforms.wiggleSpeed))
      )
      ...
      const overshoot = velocity
        .mul(hash(instanceIndex).mul(0.05).add(0.6))
        .mul(flowField.mul(1.2))
        .toVar();
      ...
      const toTargetPosition = basePosition
        .sub(position)
        .toVar("toTargetPosition");
      const toTargetPositionLength = toTargetPosition
        .length()
        .toVar("toTargetPositionLength");
      const toTargetPositionDirection = toTargetPosition
        .normalize()
        .toVar("toTargetPositionDirection");
